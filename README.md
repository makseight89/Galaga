 ...
 + Основные классы: 
   ...
   +  В папке assets находятся отображаемые объекты: 
    ...
    + Класс Game определяет основные методы, который представляет основной цикл игры: 
        ...
        + _init_ инициализирует основные атрибуты объекта: done (завершена ли игра), screen (игровой экран), clock (объект для времени), fps (количество кадров в секунду), states (словарь текущих состояний), state_name (имя текущего состояния) и state (текущее игровое состояние).
          
        + event_loop (self) – метод обрабатывающий игровые события. 
  
        + update (self, dt) - метод для обновления игрового состояния.
     
        + draw(self) - Метод для отрисовки игрового состояния на экране.
     
        + run(self) - метод для запуска игрового цикла.
    
    + Папка states содержат основные классы для управления игрой: base_state, game_over, gameplay, menu, splash. 
        
        + BaseState представляет основные методы и атрибуты для игровых состояний в игре: _init (self), startup (self), get_event (self, event), update (self, event), draw (self, surface).
     
        + Класс GameOver является подклассом и представляет игровое состояние «Конец игры»: _init_ (self), get_event (self, event), draw (self, surface).
        ...
        + Класс Splash представляет состояние «заставки» игры, которое отображает заголовок на экране: _init_, update, draw.  
    ...
    + Класс SpriteSheet предназначен для работы с спрайтами (графическими изображениями) в виде листа или стрипа.
      ...
      ![image1](https://github.com/makseight89/Galaga/blob/main/%D0%B3%D0%BB%D0%B0%D0%B2%D0%BD%D1%8B%D0%B9%20%D0%BA%D0%BB%D0%B0%D1%81%D1%81.jpg)
