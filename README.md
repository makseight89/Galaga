...
Функция align_all выравнивает контрольные точки на основе координат. 
...
Рисунок 1 — Передвижение корабля (moving)

Отображение объектов (sprites): 
ControlPoint –  класс, который представляет отображение точки в графическом интерфейсе:
__init__ инициализирует различные атрибуты: координаты точек, индексы, коллекцию контрольных точек и объект перемещения контрольной точки.
...
__init__ инициализирует различные атрибуты противника: таймер, набор контрольных точек, интервал между кадрами, отображение, вычисление точек пути.
...
_init_ инициализирует различные атрибуты взрыва: таймер, интервал между изменениями кадров, количество изображений взрыва, список изображений, список текущих отображений.
sprites. Load _strip – загрузка изображений взрыва
...
_init_ инициализирует различные атрибуты игрока: таймер, интервал между изменениями кадров, количество изображений игрока, cписок изображений и индекс текущего изображения.
...
__init__ инициализирует различные атрибуты ракеты: таймер, интервал между кадрами, количество изображений ракеты, вертикальная скорость, горизонтальная скорость, список изображений, угол поворота.
...
Рисунок 2 — Отображение объектов
Общие параметры (States): 
Game – класс, который управляет состояниями игры и отвечает за её запуск:
__init__ инициализирует различные атрибуты игры: флаг для указания конца игры, поверхность для отображения игрового окна, объект для управления временем, словарь для хранения состояний игры и имя текущего состояния.
SpriteSheet – класс, который работает с листами отображений:
__init__ является конструктором класса SpriteSheet. Он загружает спрайт-лист из файла и сохраняет его в атрибут. Если файл не найден, выводится сообщение об ошибке.
...
__init__ создает три коллекции звезд, которые содержат определённое количество звёзд в случайных позициях на экране.
...
Рисунок 3 — Общие параметры (states)
